...
def get_data_for_array(data: list) -> tuple:
    from_values = []
    prices = []
    queries = []
    num_of_the_rating = []
    ...
    return (from_values, prices, queries, num_of_the_rating, marks, count_marks, fbo)

def create_array(data: tuple) -> tuple:
    from_values, prices, queries, num_of_the_rating, marks, count_marks, fbo = *data
...
def create_one_not_encoding_dict(queries) -> np.array:
...
def normalize_data(from_values, num_of_the_rating, marks, count_marks, fbo):
...
def concatenate_priznaks(from_values, num_of_the_rating, marks, count_marks, fbo, prices):
...
# Сохранение модели
model.save_model("neuro/xgboost_model.json")
